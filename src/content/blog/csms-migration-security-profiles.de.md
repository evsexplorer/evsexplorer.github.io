...
schreibt mit `SetNetworkProfileRequest` (B09) ein neues Profil in einen freien
...
die Station mit dem neuen Backend. Scheitert das neue Profil nach
...
einziger Hebel ist das, was die Ladestation lokal anbietet, über ein Web-UI, eine
Bluetooth-App oder eine Konfigurationsdatei.

Und was er dort lokal eingeben muss, hängt vollständig vom Security Profile ab.
...
(A00.FR.201), und die OCA benennt die Konsequenz deutlich: ist das VPN die
...
Hier beginnen Migrationen, still zu scheitern. Die Station muss den
...
Die naheliegende Frage ist, ob die Box der öffentlichen CA, die das neue
...
- das neue CSMS präsentiert ein Zertifikat, dessen Kette auf einen Root führt,
  der bereits in der Box liegt (über Betreibergrenzen hinweg selten, realistisch
...
- jemand mit bestehender OCPP-Verbindung installiert es vorher per
  `InstallCertificateRequest` (Use Case M05), was bedeutet, dass das **alte**
  CSMS mitspielen muss.

Drei weitere Punkte beißen im Feld. Wildcard-Zertifikate sind nicht
...
`CSMSRootCertificate` (plus ein temporäres Fallback) installiert sein, und ein
...
Hersteller-Apps und lokale Web-UIs wurden für genau eine Aufgabe gebaut: bei der
Inbetriebnahme OCPP-URL und Zugangsdaten einzutragen. Zertifikatsverwaltung galt
als Sache des CPO, erledigt über OCPP, aus einem Backend, das mitspielt. Genau
diese Annahme bricht, wenn der Migrierende der Eigentümer ist und das alte
Backend kein Interesse daran hat, zu helfen.

Das Ergebnis ist ein Gerät, das technisch problemlos mit einem anderen CSMS
sprechen könnte, hinter einem UI, das diese Änderung nicht ausdrücken kann.
...
- Verlässliche Zeit vor dem ersten TLS-Versuch, über NTP oder besser NTS.
